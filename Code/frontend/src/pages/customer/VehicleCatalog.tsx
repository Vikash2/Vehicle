    ...
    <div className="min-h-screen bg-[var(--bg-secondary)] pt-0 pb-20">
      ...
      <div className="bg-[var(--bg-primary)] border-b border-[var(--border)] pt-8 pb-4 sticky top-0 md:top-16 z-30 transition-colors">
        <div className="container-custom flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
          <div>
            <h1 className="text-3xl font-black text-[var(--text-primary)]">Vehicle Catalog</h1>
            <p className="text-[var(--text-secondary)] font-medium">Showing {filteredVehicles.length} vehicles matching your criteria</p>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
             ...
             <select 
               className="bg-[var(--bg-secondary)] border-none rounded-xl px-4 py-2.5 font-bold text-[var(--text-primary)] outline-none focus:ring-2 focus:ring-red-500 w-full md:w-48 appearance-none cursor-pointer shadow-sm"
               value={sort}
      ...
      <div className="container-custom mt-8 flex gap-8 relative items-start">
        ...
        {/* Sidebar Filters */}
        <aside className={`fixed md:sticky top-0 right-0 h-full md:h-[calc(100vh-140px)] w-[300px] shrink-0 bg-[var(--card-bg)] md:bg-transparent border-l border-[var(--border)] md:border-none p-6 md:p-0 overflow-y-auto transform transition-transform duration-300 z-50 md:z-auto ${showFilters ? 'translate-x-0' : 'translate-x-full md:translate-x-0'}`}>
           <div className="flex justify-between items-center md:hidden mb-6">
              <h3 className="font-black text-lg flex items-center gap-2 text-[var(--text-primary)]"><SlidersHorizontal size={20}/> Filters</h3>
              <button className="text-[var(--text-muted)] p-2" onClick={() => setShowFilters(false)}><X size={20}/></button>
           </div>

           <div className="space-y-8 pr-2">
              {/* Categories */}
              <div>
                 <h4 className="text-sm font-black text-[var(--text-muted)] uppercase tracking-widest mb-4">Category</h4>
                 <div className="space-y-2">
                    {categories.map(cat => (
                       <label key={cat} className="flex items-center gap-3 cursor-pointer group">
                          ...
                          />
                          <span className={`${category === cat ? 'font-black text-[var(--text-primary)]' : 'font-bold text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]'}`}>{cat}</span>
                       </label>
                    ))}
                 </div>
              </div>
              ...
              <div className="pt-6 border-t border-[var(--border)]">
                 <h4 className="text-sm font-black text-[var(--text-muted)] uppercase tracking-widest mb-4 flex justify-between">
                    <span>Max Price</span>
              ...
              {/* Features */}
              <div className="pt-6 border-t border-[var(--border)]">
                 <h4 className="text-sm font-black text-[var(--text-muted)] uppercase tracking-widest mb-4">Must-Have Features</h4>
                 <div className="space-y-3">
                    {allFeatures.map(feat => (
                       <label key={feat} className="flex items-center gap-3 cursor-pointer group">
                          ...
                          />
                          <span className={`font-bold ${selectedFeatures.includes(feat) ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]'}`}>{feat}</span>
                       </label>
                    ))}
                 </div>
              </div>
           ...
           <div className="mt-8 pt-6 border-t border-[var(--border)] md:hidden">
        ...
        <div className="flex-1 w-full min-w-0">
           {filteredVehicles.length === 0 ? (
              <div className="text-center py-24 bg-[var(--card-bg)] rounded-3xl border border-dashed border-[var(--border)]">
                 <h3 className="text-xl font-black text-[var(--text-primary)] mb-2">No vehicles found</h3>
                 <p className="text-[var(--text-secondary)] mb-6">Try relaxing your filters or expanding the price range.</p>
