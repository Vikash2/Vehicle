    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 w-full md:w-auto no-scrollbar">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-lg font-bold text-sm whitespace-nowrap transition-all ${selectedCategory === cat ? 'bg-red-50 text-red-600' : 'text-[var(--text-muted)] hover:bg-[var(--hover-bg)]'}`}
            >
              {cat}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredAccessories.map(acc => (
          <div key={acc.id} className="group bg-[var(--card-bg)] rounded-2xl p-6 border border-[var(--border)] shadow-sm hover:shadow-xl hover:border-red-200 transition-all duration-300 relative overflow-hidden">
             {/* Status Badge */}
             <div className="absolute top-4 right-4 z-10">
                {acc.inStock ? (
                  <span className="bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-full border border-emerald-100 flex items-center gap-1">
                    <Check size={10} /> In Stock
                  </span>
                ) : (
                  <span className="bg-red-50 text-red-600 text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-full border border-red-100 flex items-center gap-1">
             ...
             <div className="flex items-start gap-4 mb-6">
                <div className="w-14 h-14 bg-[var(--bg-secondary)] rounded-2xl flex items-center justify-center text-[var(--text-muted)] group-hover:bg-red-50 group-hover:text-red-500 transition-colors shadow-inner">
                   {getCategoryIcon(acc.category)}
                </div>
                <div className="flex-1 pr-12">
                   <h3 className="font-black text-[var(--text-primary)] group-hover:text-red-600 transition-colors truncate">{acc.name}</h3>
                   <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">{acc.category}</p>
                </div>
             </div>

             <p className="text-sm text-[var(--text-secondary)] mb-6 line-clamp-2 min-h-[2.5rem] font-medium leading-relaxed">
                {acc.description}
             </p>

             <div className="flex items-end justify-between border-t border-[var(--border)] pt-6">
                <div>
                  <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-1">Total Price</p>
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-black text-[var(--text-primary)]">₹{acc.price.toLocaleString('en-IN')}</span>
                    {acc.installationCharges > 0 && <span className="text-xs text-[var(--text-muted)] font-bold">+₹{acc.installationCharges} fitting</span>}
                  </div>
                </div>
                <div className="flex gap-2">
                   <button 
                     onClick={() => openEditModal(acc)}
                     className="p-2 hover:bg-[var(--hover-bg)] rounded-lg text-[var(--text-muted)] hover:text-blue-500 transition-colors"
                   >
                     <Edit2 size={18} />
                   </button>
                   <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--modal-overlay)] backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-[var(--card-bg)] w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-300">
              <div className="p-8 border-b border-[var(--border)] flex justify-between items-center">
                 ...
                 <button onClick={closeModal} className="p-2 hover:bg-[var(--hover-bg)] rounded-full text-[var(--text-muted)] transition-colors">
                    <X size={24} />
                 </button>
              </div>
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                 <div>
                    ...
                    <input 
                      required
                      type="text" 
                      className="w-full bg-[var(--bg-secondary)] border-none rounded-xl px-4 py-3 font-medium focus:ring-2 focus:ring-red-500 transition-all text-[var(--text-primary)]"
                      value={formData.name}
                 ...
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                       ...
                       <select 
                         className="w-full bg-[var(--bg-secondary)] border-none rounded-xl px-4 py-3 font-medium focus:ring-2 focus:ring-red-500 transition-all appearance-none text-[var(--text-primary)]"
                    ...
                    <div>
                       ...
                       <div className="flex h-[48px] bg-[var(--bg-secondary)] rounded-xl p-1">
                          <button 
                            type="button"
                            onClick={() => setFormData({...formData, inStock: true})}
                            className={`flex-1 rounded-lg font-bold text-xs transition-all ${formData.inStock ? 'bg-emerald-500 text-white shadow-md' : 'text-[var(--text-muted)] hover:text-[var(--text-secondary)]'}`}
                          >
                            In Stock
                          </button>
                          <button 
                            ...
                            className={`flex-1 rounded-lg font-bold text-xs transition-all ${!formData.inStock ? 'bg-red-500 text-white shadow-md' : 'text-[var(--text-muted)] hover:text-[var(--text-secondary)]'}`}
                          >
                            Out of Stock
                          </button>
                       </div>
                 ...
                 <div>
                    ...
                    <textarea 
                      required
                      className="w-full bg-[var(--bg-secondary)] border-none rounded-xl px-4 py-3 font-medium focus:ring-2 focus:ring-red-500 transition-all min-h-[80px] text-[var(--text-primary)]"
                 ...
                 <div className="pt-4 flex gap-4">
                    <button type="button" onClick={closeModal} className="flex-1 px-6 py-3 rounded-xl font-bold text-[var(--text-muted)] hover:bg-[var(--hover-bg)] transition">Cancel</button>
