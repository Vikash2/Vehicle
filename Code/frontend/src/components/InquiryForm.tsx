...
const InquiryForm = ({ source = 'Website', onSuccess, compact = false }: InquiryFormProps) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={compact ? 'space-y-4' : 'bg-[var(--card-bg)] rounded-2xl p-8 shadow-xl border border-[var(--border)] space-y-6'}>
      {!compact && (
        <div className="text-center mb-6">
          <h3 className="text-2xl font-black text-[var(--text-primary)] mb-2">Get in Touch</h3>
          <p className="text-[var(--text-secondary)] font-medium">Fill out the form and we'll get back to you soon</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 text-[var(--text-primary)] placeholder:text-[var(--text-muted)]"
          />
          {errors.fullName && <p className="text-red-500 text-xs mt-1 font-medium">{errors.fullName.message}</p>}
        </div>

        <div>
          <label className="block text-sm font-bold text-[var(--text-secondary)] mb-2">Mobile Number *</label>
          <input
        ...
        <div>
          <label className="block text-sm font-bold text-[var(--text-secondary)] mb-2">Email (Optional)</label>
          <input
            {...register('email')}
            type="email"
        ...
        <div>
          <label className="block text-sm font-bold text-[var(--text-secondary)] mb-2">City *</label>
          <input
            {...register('city')}
            type="text"
      ...
      <div>
        <label className="block text-sm font-bold text-[var(--text-secondary)] mb-2">Interested Vehicle *</label>
        <select
          {...register('interestedModel')}
          className="w-full px-4 py-3 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 text-[var(--text-primary)]"
        >
        ...
        {errors.interestedModel && <p className="text-red-500 text-xs mt-1 font-medium">{errors.interestedModel.message}</p>}
      </div>

      <div>
        <label className="block text-sm font-bold text-[var(--text-secondary)] mb-2">Purchase Timeline *</label>
        <select
          {...register('timeline')}
          className="w-full px-4 py-3 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500 text-[var(--text-primary)]"
        >
      ...
      <div className="space-y-3">
        <p className="text-sm font-bold text-[var(--text-secondary)]">Additional Requirements</p>
            ...
            return (
              ...
              >
                <input
                  ...
                  className="sr-only"
                />
                <Icon size={20} className={isActive ? 'text-red-600' : 'text-[var(--text-muted)]'} />
                <span className={`text-sm font-bold ${isActive ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}`}>
                  {option.label}
                </span>
              </label>
            );
      ...
      <button
        ...
        className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-4 rounded-xl transition-all shadow-lg shadow-red-600/20 disabled:opacity-50 disabled:cursor-not-allowed"
      >
